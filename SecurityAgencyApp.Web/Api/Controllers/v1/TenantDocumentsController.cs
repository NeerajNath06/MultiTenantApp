...
public class TenantDocumentsController : ControllerBase
{
    ...
    public async Task<IActionResult> ExportDocuments([FromQuery] string format = "csv", CancellationToken cancellationToken = default)
    {
        ...
        if (!result.Success || result.Data == null)
            return NotFound(ApiResponse<object?>.ErrorResponse("Tenant documents not found"));
        ...
        [FromForm] IFormFile? file)
    {
        ...
        if (string.IsNullOrWhiteSpace(documentType))
            return BadRequest(ApiResponse<Guid>.ErrorResponse(
                "Validation failed",
                new[] { ApiError.Create("DocumentType is required", "documentType") }));
        if (file == null || file.Length == 0)
            return BadRequest(ApiResponse<Guid>.ErrorResponse(
                "Validation failed",
                new[] { ApiError.Create("File is required", "file") }));
    ...
    public async Task<IActionResult> DownloadDocument(Guid id)
    {
        ...
        if (!System.IO.File.Exists(fullPath))
            return NotFound(ApiResponse<object?>.ErrorResponse("Document file not found"));
