...
[Authorize]
public class IncidentsController : GenericCrudControllerBase<
    ...
    DeleteIncidentCommand>
{
    ...
    protected override GetIncidentByIdQuery CreateGetByIdQuery(Guid id) => new() { Id = id };

    protected override void SetUpdateCommandId(UpdateIncidentCommand command, Guid id) => command.Id = id;

    protected override DeleteIncidentCommand CreateDeleteCommand(Guid id) => new() { Id = id };
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (fmt != "csv" && fmt != "xlsx" && fmt != "pdf")
            return BadRequest(ApiResponse<object?>.ErrorResponse(
                "Validation failed",
                new[] { ApiError.Create("Supported format: csv, xlsx, pdf", "format") }));
        ...
        if (!result.Success || result.Data == null)
            return NotFound(ApiResponse<object?>.ErrorResponse("Incident data not found"));
