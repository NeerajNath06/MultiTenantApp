...
public class DeploymentsController : ControllerBase
{
        ...
        [FromQuery] int pageSize = 500)
    {
        ...
        return Ok(ApiResponse<List<DeploymentDto>>.SuccessResponse(list));
    }
        ...
        [FromQuery] Guid? supervisorId = null)
    {
        ...
        {
            return BadRequest(ApiResponse<RosterDto>.ErrorResponse(
                "Validation failed",
                new[]
                {
                    ApiError.Create("dateFrom is required in yyyy-MM-dd format.", "dateFrom"),
                    ApiError.Create("dateTo is required in yyyy-MM-dd format.", "dateTo")
                }));
        }
        if (fromDate > toDate)
        {
            return BadRequest(ApiResponse<RosterDto>.ErrorResponse(
                "Validation failed",
                new[] { ApiError.Create("dateFrom must be less than or equal to dateTo.", "dateFrom") }));
        }
        ...
        return Ok(ApiResponse<RosterDto>.SuccessResponse(roster));
    }
}
