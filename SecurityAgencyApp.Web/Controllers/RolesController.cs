...
public class RolesController : Controller
{
    ...
    {
        var result = await _apiClient.GetAsync<PermissionListResponse>("Permissions", new Dictionary<string, string?> { ["includeInactive"] = "false" });
    ...
    public async Task<IActionResult> Index(int pageNumber = 1, int pageSize = 10, string? search = null, string? sortBy = null, string? sortDirection = "asc")
    {
        ...
        var result = await _apiClient.GetAsync<RoleListResponse>("Roles", query);
    ...
    public async Task<IActionResult> Create(CreateRoleRequest request)
    {
        ...
        var result = await _apiClient.PostAsync<Guid>("Roles", new { name = request.Name, code = request.Code, description = request.Description });
        if (result.Success)
        {
    ...
    {
        var result = await _apiClient.GetAsync<RoleDto>($"Roles/{id}");
        if (!result.Success || result.Data == null)
            return NotFound();
        await LoadPermissionsAsync();
        var d = result.Data;
    ...
    public async Task<IActionResult> Edit(UpdateRoleRequest request)
    {
        ...
        var result = await _apiClient.PutAsync<object>($"Roles/{request.Id}", new { id = request.Id, name = request.Name, code = request.Code, description = request.Description, isActive = request.IsActive, permissionIds = request.PermissionIds });
        if (result.Success)
        {
    ...
    {
        var result = await _apiClient.DeleteAsync($"Roles/{id}");
        if (result.Success)
    ...
    {
        var roleResult = await _apiClient.GetAsync<RoleDto>($"Roles/{id}");
        if (!roleResult.Success || roleResult.Data == null)
            return NotFound();
        var permResult = await _apiClient.GetAsync<PermissionListResponse>("Permissions", new Dictionary<string, string?> { ["includeInactive"] = "false" });
    ...
    {
        var result = await _apiClient.PostAsync<object>($"Roles/{id}/assign-permissions", permissionIds ?? new List<Guid>());
        if (result.Success)
        {
    ...
    public async Task<IActionResult> ManageMenus(Guid id)
    {
        ...
        if (!roleResult.Success || roleResult.Data == null)
            return NotFound();
        var menuResult = await _apiClient.GetAsync<MenuListResponse>("Menus", new Dictionary<string, string?> { ["includeInactive"] = "false", ["pageSize"] = "100" });
        ViewBag.Role = roleResult.Data;
    ...
    public async Task<IActionResult> ManageMenus(Guid id, List<Guid> menuIds, List<Guid> subMenuIds)
    {
        ...
        var result = await _apiClient.PostAsync<object>($"Menus/{id}/assign", body);
        if (result.Success)
        {
